...
void SimplePlanningSimulator::on_timer()
{
  ...
  [[maybe_unused]] int32_t sent_PTCL =
    convert_to_PTCL(current_odometry_, current_velocity_, current_steer_);
  // if(sent_PTCL == EXIT_VALUE_OK){
  //   // RCLCPP_ERROR(this->get_logger(), "send PTCL succeeded");
  // }else if(sent_PTCL == EXIT_VALUE_ERR)  {
  //   RCLCPP_ERROR(this->get_logger(), "send PTCL failed");
  // }else{
  //   RCLCPP_ERROR(this->get_logger(), "what's going on");
  // }
...
int32_t SimplePlanningSimulator::convert_to_PTCL(
  const Odometry & odometry, const VelocityReport & velocity, const SteeringReport & steer)
{
  ...
  CarState.header.timeReference = PTCL_toPTCLTime(current_time);
  CarState.header.vehicleId = 1;
  CarState.pose.position.x = position_x_PTCL;
  CarState.pose.position.y = position_y_PTCL;
  const double yaw_pose = tf2::getYaw(odometry.pose.pose.orientation);
  CarState.pose.heading = PTCL_toPTCLAngleWrapped(yaw_pose);
  CarState.angleRateYaw = PTCL_toPTCLAngleRate(odometry.twist.twist.angular.z);
  CarState.velLon = PTCL_toPTCLSpeed(velocity.longitudinal_velocity);
  CarState.velLat = PTCL_toPTCLSpeed(velocity.lateral_velocity);
  CarState.accelLon = PTCL_toPTCLAccel(0);
  CarState.gear = PTCL_GEAR_SELECTOR_D;
  CarState.controllerMode.latControlActive = true;
  CarState.controllerMode.lonControlActive = true;
  CarState.angleSteeredWheels = PTCL_toPTCLAngleWrapped(steer.steering_tire_angle);
  CarState.numSemiTrailers = static_cast<uint8_t>(0);

  // if (portInterfaceSender != NULL)
  // {
  //     // RCLCPP_ERROR(this->get_logger(),
  //     // "Initialized autoware UDP port with PTCL_Id %u.\n", AUTOWARE_ID);
  //     // printf("Initialized autoware UDP port with PTCL_Id %u.\n", AUTOWARE_ID);
  // }else{
  //     RCLCPP_ERROR(this->get_logger(),
  //     "Init of autoware context failed.\n");
  //     // printf("Init of autoware context failed.\n");
  // }
  // // Setup prodriver callback
  // const PTCL_Id sourceId = AUTOWARE_ID;
  // PTCL_PerceptionFrameReceiverContext perceptionFrameReceiverContext;
  // const bool installSuccess = PTCL_CarState_installCallback(
  //     &contextReceiver, perceptionFrame_CB, &exampleData, sourceId,
  //     &perceptionFrameReceiverContext);
  // ID <> UDP host address map
  ...
  // printf("Starting PTCL UDP port example.\n");
  ...
  if (!setupSuccess) {
    RCLCPP_ERROR(this->get_logger(), "Failed to initialize vehicle UDP port.\n");
  } else {
    RCLCPP_ERROR(this->get_logger(), "Succeeded to initialize vehicle UDP port.\n");
  }

  // Init ExampleData struct
  // static CarStateData carStateData;
  // memset(&carStateData, 0, sizeof(CarStateData));
  // Send message
  // carStateData.msgReceived = false;
  // carStateData.msg = CarState;

  const PTCL_Id destinationId = PRODRIVER_ID;
  [[maybe_unused]] const bool sendCarStateSuccess =  //
    PTCL_CarState_send(&contextCarStateSender, &CarState, destinationId);

  // if (sendCarStateSuccess)
  // {
  //   exitValue = EXIT_VALUE_OK;
  //   RCLCPP_ERROR(this->get_logger(), "car state sent.\n");

  // }else{
  //     RCLCPP_ERROR(this->get_logger(), "Sending message to prodriver failed.\n");
  // }
  // }else{
  //     RCLCPP_ERROR(this->get_logger(), "Initialization of PTCL file port for car state failed.");
  // }
  PTCL_UdpPort_destroy(&udpPortCarStateSender);
  // RCLCPP_ERROR(this->get_logger(), "convert_to_PTCL end");
  return exitValue;
}

void SimplePlanningSimulator::convert_pose_to_PTCL_coordinate(
  const lanelet::GPSPoint & p_origin, const lanelet::GPSPoint & p_target, int32_t & position_x_PTCL,
  int32_t & position_y_PTCL)
{
